...
GLuint TextureManager::LoadTextureFromMemory(std::vector<unsigned char> dataImage)
{


    int width, height, channels;
    unsigned char* data = stbi_load_from_memory(dataImage.data(), dataImage.size(), &width, &height, &channels, 4);

    if (!data) {

        std::cout << "Текстура не загружена:" << std::endl;
        return placeholderTexture;  // Возвращаем заглушку
    }
    ...
    // Кэшируем текстуру
   // textureCacheMemory[dataImage] = textureID;
    return textureID;
